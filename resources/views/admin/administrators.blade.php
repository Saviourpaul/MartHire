<x-layout>
    ...
    <div class="border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
        ...
        @endif
        <!-- DataTable -->
        <div data-table-refresh-region data-table-refresh-id="admin-administrators" class="overflow-hidden rounded-xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="max-w-full overflow-x-auto">
                <div class="min-w-[1102px]">
                    ...
                    @forelse ($users as $user)
                        <div x-data="{ isViewUserModal{{ $user->id }}: false, isEditUserModal{{ $user->id }}: false, isDeleteUserModal{{ $user->id }}: false }">
                        <div class="grid grid-cols-12 border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900">
                            ...
                            <div class="col-span-1 flex items-center gap-2 px-4 py-3">
                                <button
                                    x-on:click="isViewUserModal{{ $user->id }} = true"
                                ...
                                <button
                                    x-on:click="isEditUserModal{{ $user->id }} = true"
                                    class="text-gray-500 hover:text-brand-600 dark:text-gray-400 dark:hover:text-brand-400">
                                    <svg class="fill-current" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <button
                                    x-on:click="isDeleteUserModal{{ $user->id }} = true"
                        ...
                        <!-- View User Modal -->
                        <div x-show="isViewUserModal{{ $user->id }}" x-cloak class="fixed inset-0 z-99999 flex items-center justify-center overflow-y-auto p-4 sm:p-5">
                            <div class="modal-close-btn fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px]"></div>
                            <div @click.outside="isViewUserModal{{ $user->id }} = false" class="no-scrollbar relative max-h-[calc(100vh-2rem)] w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 sm:p-6 lg:p-11 dark:bg-gray-900">
                                <button type="button" @click="isViewUserModal{{ $user->id }} = false" class="transition-color absolute top-5 right-5 z-999 flex h-11 w-11 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:bg-gray-700 dark:bg-white/[0.05] dark:text-gray-400 dark:hover:bg-white/[0.07] dark:hover:text-gray-300" aria-label="Close user profile modal">
                        ...
                        <!-- Edit User Modal -->
                        <div x-show="isEditUserModal{{ $user->id }}" x-cloak class="fixed inset-0 z-99999 flex items-center justify-center overflow-y-auto p-4 sm:p-5">
                            <div class="modal-close-btn fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px]"></div>
                            <div @click.outside="isEditUserModal{{ $user->id }} = false" class="no-scrollbar relative max-h-[calc(100vh-2rem)] w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 sm:p-6 lg:p-11 dark:bg-gray-900">
                                <button type="button" @click="isEditUserModal{{ $user->id }} = false" class="transition-color absolute top-5 right-5 z-999 flex h-11 w-11 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:bg-gray-700 dark:bg-white/[0.05] dark:text-gray-400 dark:hover:bg-white/[0.07] dark:hover:text-gray-300" aria-label="Close edit user modal">
                                    <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M6.04289 16.5413C5.65237 16.9318 5.65237 17.565 6.04289 17.9555C6.43342 18.346 7.06658 18.346 7.45711 17.9555L11.9987 13.4139L16.5408 17.956C16.9313 18.3466 17.5645 18.3466 17.955 17.956C18.3455 17.5655 18.3455 16.9323 17.955 16.5418L13.4129 11.9997L17.955 7.4576C18.3455 7.06707 18.3455 6.43391 17.955 6.04338C17.5645 5.65286 16.9313 5.65286 16.5408 6.04338L11.9987 10.5855L7.45711 6.0439C7.06658 5.65338 6.43342 5.65338 6.04289 6.0439C5.65237 6.43442 5.65237 7.06759 6.04289 7.45811L10.5845 11.9997L6.04289 16.5413Z" fill="" /></svg>
                                </button>
                        ...
                        <!-- Delete User Modal -->
                        <div x-show="isDeleteUserModal{{ $user->id }}" x-cloak class="fixed inset-0 z-99999 flex items-center justify-center overflow-y-auto p-4 sm:p-5">
                            <div class="modal-close-btn fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px]"></div>
                            <div @click.outside="isDeleteUserModal{{ $user->id }} = false" class="no-scrollbar relative max-h-[calc(100vh-2rem)] w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 sm:p-6 lg:p-11 dark:bg-gray-900">
                                <button type="button" @click="isDeleteUserModal{{ $user->id }} = false" class="transition-color absolute top-5 right-5 z-999 flex h-11 w-11 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:bg-gray-700 dark:bg-white/[0.05] dark:text-gray-400 dark:hover:bg-white/[0.07] dark:hover:text-gray-300" aria-label="Close delete user modal">
                                    <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M6.04289 16.5413C5.65237 16.9318 5.65237 17.565 6.04289 17.9555C6.43342 18.346 7.06658 18.346 7.45711 17.9555L11.9987 13.4139L16.5408 17.956C16.9313 18.3466 17.5645 18.3466 17.955 17.956C18.3455 17.5655 18.3455 16.9323 17.955 16.5418L13.4129 11.9997L17.955 7.4576C18.3455 7.06707 18.3455 6.43391 17.955 6.04338C17.5645 5.65286 16.9313 5.65286 16.5408 6.04338L11.9987 10.5855L7.45711 6.0439C7.06658 5.65338 6.43342 5.65338 6.04289 6.0439C5.65237 6.43442 5.65237 7.06759 6.04289 7.45811L10.5845 11.9997L6.04289 16.5413Z" fill="" /></svg>
                                </button>
                            <div class="p-6">
                                ...
                                <form method="post" action="{{ route('admin.users.destroy', $user) }}" class="mt-6">
                                    ...
                                    <div class="mt-6 flex items-center justify-end gap-3">
                                        <x-secondary-button type="button" @click="isDeleteUserModal{{ $user->id }} = false">
                                            Cancel
                                        </x-secondary-button>
