            ...
            <div class="mb-4 flex flex-col gap-2 px-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <form method="GET" action="{{ url()->current() }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ $filterValues['search'] }}" placeholder="Title, employer, category..."
                    ...
                    <div class="col-lg-2 col-md-6">
                        ...
                        <input class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 rounded-lg border border-gray-300 bg-transparent px-3 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                       name="submitted_from" type="date"
                            value="{{ $filterValues['submitted_from'] }}">
                    </div>
                    <div class="col-lg-2 col-md-6">
                        <label class="text-gray-500 dark:text-gray-400">Submitted To</label>
                        <input class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 rounded-lg border border-gray-300 bg-transparent px-3 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                       name="submitted_to" type="date"
                            value="{{ $filterValues['submitted_to'] }}">
                    </div>

                    <select name="category" onchange="this.form.submit()" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 rounded-lg border border-gray-300 bg-transparent px-3 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    ...
                    @forelse ($jobs as $job)
                        <div class="grid grid-cols-12 border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900">
                            <div class="col-span-3 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                                <div class="flex gap-3">
                                    <div class="h-10 w-10 overflow-hidden rounded-full">
                                        <img src="{{ $job->logoUrl() }}" alt="{{ $job->logoUrl() }}" class="h-full w-full object-cover" />
                                    </div>
                                    <div>
                            ...
                            <div class="col-span-2 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                                <p class="text-theme-sm text-gray-700 dark:text-gray-400 truncate" title="">{{ $job->employer?->first_name }} {{ $job->employer?->last_name }}</p>
                            </div>
                            <div class="col-span-2 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                                <p class="text-theme-sm text-gray-700 dark:text-gray-400 truncate" title="">{{ $job->employer?->email ?? 'Employer unavailable' }}</p>
                            </div>
                            <div class="col-span-1 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                                <p class="text-theme-sm text-gray-700 dark:text-gray-400">{{ $job->category ?: 'Uncategorized' }}</p>
                            </div>
                            <div class="col-span-1 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                                <p class="text-theme-sm text-success-600  {{ $job->status->label() }}">
                                     {{ $job->status->label() }}
                                </p>
                            </div>
                             <div class="col-span-1 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                            ...
                            <div class="col-span-1 flex items-center gap-2 px-4 py-3">
                                <button
                                    x-on:click="$dispatch('open-modal', 'view-user-"
                                ...
                                <button
                                    x-on:click="$dispatch('open-modal', 'edit-user-"
                                    class="text-gray-500 hover:text-brand-600 dark:text-gray-400 dark:hover:text-brand-400">
                                    <svg class="fill-current" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
